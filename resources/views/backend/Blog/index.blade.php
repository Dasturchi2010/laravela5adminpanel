...
@section('content')
    ...
    <div class="col-12 text-right">
        <a href="{{ route('blogs.create') }}" class="btn btn-success btn-sm mb-1">Qo'shish</a>
    </div>
            ...
            <tbody>
                @foreach ($blogs as $blog)
                    <tr>
                        ...
                        <td>{{ $blog->title }}</td>
                        <td>{{Str::limit($blog->mini_text , 20)}}</td>
                        <td>
                            <img width="40" height="40" src="{{asset('/storage/' . $blog->photo)}}" alt="img error">
                        </td>
                        <td>{{ $blog->author}}</td>
                    <td>
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu" style="">
                                ...
                                <form action="{{ route('blogs.destroy', $blog->id) }}" method="POST" style="display: inline;">
                                    @csrf
                                    @method('DELETE')
                                    <button style="width: 100%" type="submit" class="dropdown-item" onclick="return confirm('blog o\'chirmoqchimisiz')">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14"
                                            viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
